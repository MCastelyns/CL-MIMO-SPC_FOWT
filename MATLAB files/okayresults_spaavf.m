...
% spaavf
% Use small Nband to include very low frequencies in the first averaged bin.
Nband = 6;                        % lower => includes lower freqs
Nfft  = 2^nextpow2(Tbatch);       % FFT length
[Gspa, wspa] = spaavf(u_batch, y_batch, Ts, Nband, Nfft);
sys_spa = [];
% Sometimes returns empty objects, causes bugs later
...
figure('Color','w');
% bodeplot(G_final,{wmin,wmax},'r-.',opt);  % still hidden
bodeplot(sysd(idx_y,idx_u),{wmin,wmax},'k',opt); hold on;           % True
bodeplot(G_batch_strict,{wmin,wmax},'b--',opt);                     % Batch ARX
if ~isempty(sys_spa)
    bodeplot(sys_spa, {wmin,wmax}, opt);                            % SPA-AVF
end
legend('True','Batch ARX','SPA-AVF (batch)','Location','best');
